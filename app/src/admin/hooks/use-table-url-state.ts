...
export function useTableUrlState(
  ...
  const onPaginationChange: OnChangeFn<PaginationState> = (updater) => {
    ...
    // navigate(`?${new URLSearchParams({ page: String(nextPage) }).toString()}`)
    const searchParams = new URLSearchParams(window.location.search)
    searchParams.set(pageKey, String(nextPage))
    searchParams.set(pageSizeKey, String(nextPageSize))
  ...
  const onGlobalFilterChange: OnChangeFn<string> | undefined =
    globalFilterEnabled
      ? (updater) => {
        ...
        const searchParams = new URLSearchParams(window.location.search)
        if (value) {
          searchParams.set(globalFilterKey, value)
        } else {
          searchParams.delete(globalFilterKey)
        }
        searchParams.delete(pageKey) // reset pagination
        navigate(`?${searchParams.toString()}`)
      }
      : undefined
  ...
  const onColumnFiltersChange: OnChangeFn<ColumnFiltersState> = (updater) => {
    ...
    const searchParams = new URLSearchParams(window.location.search)
    searchParams.delete(pageKey) // reset pagination when filters change

    for (const [key, val] of Object.entries(patch)) {
      if (val === undefined) searchParams.delete(key)
  ...
  const ensurePageInRange = (
    ...
    if (pageCount > 0 && pageNum > pageCount) {
      ...
      if (opts.resetTo === 'last') {
        searchParams.set(pageKey, String(pageCount))
      } else {
