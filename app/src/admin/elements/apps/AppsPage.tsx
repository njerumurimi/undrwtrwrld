...
export function AppsPage({ user }: { user: AuthUser }) {
  ...
  const typeParam = (searchParams.get('type') as AppType) ?? 'all'
  const initSort = (searchParams.get('sort') as 'asc' | 'desc') ?? 'asc'

  const [sort, setSort] = useState<'asc' | 'desc'>(initSort)
  const [appType, setAppType] = useState<AppType>(typeParam)
  const [searchTerm, setSearchTerm] = useState<string>(filterParam)
